...
from app.utils import extract_text, ROLE_KEYWORDS
import shutil
    ...
    if st.button("Rank Resumes", type="primary"):
        ...
        try:
            ...
            if df['RelativeScore'].max() < 4:
                st.warning(f"Low scores detected. Ensure resumes mention relevant skills (e.g., '{', '.join(ROLE_KEYWORDS.get(selected_role.lower(), [])[:3])}').")
